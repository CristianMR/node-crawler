...
Crawler.prototype._buildHttpRequest = function _buildHTTPRequest(options) {
	...
	var doRequest = function (err) {
		...
		if (ropts.http2) {
			...
			var requestArgs = ['ciphers', 'uri', 'url', 'qs', 'method', 'headers', 'body', 'form', 'formData', 'json', 'multipart', 'followRedirect', 'followAllRedirects', 'maxRedirects', 'removeRefererHeader', 'encoding', 'pool', 'timeout', 'proxy', 'auth', 'oauth', 'strictSSL', 'jar', 'aws', 'gzip', 'time', 'tunnel', 'proxyHeaderWhiteList', 'proxyHeaderExclusiveList', 'localAddress', 'forever', 'agent', 'strictSSL', 'agentOptions', 'agentClass'];
...
Crawler.prototype._http2request = function _http2request(ropts, options) {
	...
	const requestBody = (ropts.headers[':method'] === 'POST' || ropts.headers[':method'] === 'PUT') ? self.generateHttp2RequestBody(ropts) : null;
	const response = {
		headers: {}
	};
